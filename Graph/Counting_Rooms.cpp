...
void dfs(int x, int y , vector<vector<char>> &a , vector<vector<bool>> &vis) {
	vis[x][y] = true;
	for (int i = 0; i < 4; i++) {
		int nx = x + dx[i];
		int ny = y + dy[i];
		if (nx >= 0 and nx<a.size() and ny >= 0 and ny < a[0].size() and vis[nx][ny] == false and a[nx][ny] == '.') {
			dfs(nx , ny , a, vis);
		}
	}
}
...
void solve(int tt)
{
	...
	for (int i = 0 ; i < n; i++) {
		for (int j = 0; j < m; j++) {
			if (vis[i][j] == false and roomMap[i][j] == '.') {
				dfs(i, j, roomMap, vis);
				countRooms++;
			}
		}
