...
void solve(int tt)
{
	...
	while (q.size()) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (nx >= 0 and nx<n and ny >= 0 and ny < m and vis[nx][ny] == false and a[nx][ny] != '#') {
				dis[nx][ny] = dis[it.first][it.second] + 1;
				parent[nx][ny] = it;
				vis[nx][ny] = true;
				q.push({nx, ny});
			}
	...
	if (vis[ed.first][ed.second]) {
		...
		string ans = "";
		while (ed != st) {
			pii p = parent[ed.first][ed.second];
			char c = getChar(ed , p);
			ed = p;
			ans += c;
		}
