...
int dx[4] = {0, 0, -1, 1};
int dy[4] = {1, -1, 0, 0};
bool rec(int i , int j, int curr ,vector<vector<char>>&a , vector<vector<bool>>&vis){
	int n = a.size();
	int m = sz(a[0]);
	if(i==n-1 and j==m-1){
		return 1;
	...
	for(int k=0;k<4;k++){
		int nx = i+dx[k];
		int ny = j+dy[k];
		if(nx>=0 and nx<n and ny>=0 and ny<m and vis[nx][ny]==false and a[nx][ny]==nxt){
			vis[nx][ny] = true;
			op|=rec(nx, ny, curr+1, a, vis);
		}
	}
	return op;
}
...
void solve(int tt)
{
	...
	vis[0][0] = 1;
	bool ans = rec(0,0 ,0, a, vis);
	if(a[0][0]!='s' or ans==false){
		No
		return;
	}
	Yes
