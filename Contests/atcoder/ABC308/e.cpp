...
unordered_map<int, vector<int>> m , e, x;
string s;
vector<int> arr;
int getMex(vector<int> &arrrrrr, int N)
{
	sort(arrrrrr.begin(), arrrrrr.end());
	int mex = 0;
...
int dp[3][MAX2][4][4];
int rec(int i , int index , int a = 3, int b = 3) {
	if (i == 2) {
		int c = arr[index];
		vector<int> hm = {a, b, c};
		return getMex(hm, 3);
	...
	if (i == 0) {
		...
		for (int k = 0; k <= 2; k++) {
			for (int j : x[k]) {
				if (j > index) {
					ans += rec(i + 1, j, a, currVal);
				}
			}
		}
	}
	return dp[i][index][a][b] = ans;
}
